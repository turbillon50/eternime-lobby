...
export async function POST(request: Request) {
  try {
    ...
    if (body?.consent !== true || typeof body.exchangeId !== "string" || !UUID.test(body.exchangeId) || typeof body.portraitId !== "string" || !UUID.test(body.portraitId)) throw new CloneError("AVATAR_INPUT", "Selecciona una respuesta y tu foto, y autoriza la generación.", 400);
    if (!process.env.HEYGEN_API_KEY) throw new CloneError("AVATAR_UNAVAILABLE", "El servicio de avatar aún no está configurado.");
    const [exchange, photo, user] = await Promise.all([getCloneExchange(session.clerkId, body.exchangeId), latestPortrait(session.clerkId), findUserById(session.sub)]);
    if (!exchange || !photo || photo.id !== body.portraitId) throw new CloneError("AVATAR_INPUT", "Actualiza la selección de tu foto y tu respuesta.", 409);
    if (exchange.clone_text.length > 350) throw new CloneError("AVATAR_LENGTH", "Pídele al clon una respuesta de hasta 350 caracteres para este video breve.", 400);
    const voiceId = personalVoiceId(user?.prefs);
    if (!voiceId) throw new CloneError("VOICE_REQUIRED", "Crea tu voz personal antes de generar el avatar.", 409);
    const { job, fresh } = await reserveJob(session.clerkId, "avatar", digest(exchange.id, exchange.clone_text, photo.id, voiceId));
    ...
    try {
      ...
      const speech = await savedSpeech(session.clerkId, voiceId, exchange.clone_text);
