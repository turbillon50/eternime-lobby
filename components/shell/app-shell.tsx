...
function SidebarProfile({ user }: { user: ShellUser | null }) {
  ...
  return (
    <Link href="/app/perfil" className={`group flex items-center gap-3 rounded-[var(--et-radius)] border p-2.5 transition ${active ? "border-[var(--et-border)] bg-[rgba(212,175,106,0.08)] shadow-[var(--et-glow)]" : "border-[var(--et-border-soft)] bg-[rgba(212,175,106,0.03)] hover:border-[var(--et-border)] hover:bg-[rgba(212,175,106,0.06)]"}`}>
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="block truncate text-xs text-[var(--et-text-faint)]">{user?.role === "admin" ? "Admin · " : ""}{user?.tagline || user?.email || ""}</span>
      </span>
      <span className="text-[var(--et-text-faint)] transition group-hover:text-[var(--et-gold)]"><Icon d="M9 6l6 6-6 6" /></span>
    </Link>
  );
}
...
function BottomBar({ user }: { user: ShellUser | null }) {
  const pathname = usePathname();
  const t = useT();
  return (
    <nav className="et-bottom-nav lg:hidden" aria-label="Navegación principal">
      {BOTTOM_NAV.map((item) => {
        const active = item.href === "/app" ? pathname === "/app" : pathname.startsWith(item.href);
        if (item.center) {
          return (
            <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} className="et-bottom-center" aria-label={t(item.label as DictKey)}>
              <span className="et-bottom-center-btn" style={{ boxShadow: active ? "var(--et-glow-strong)" : "var(--et-glow)" }}>{item.icon}</span>
            </Link>
          );
        }
        return (
          <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined}
            className={`et-bottom-item ${active ? "et-bottom-item-active" : ""}`}>
            {item.icon}
            <span className="et-bottom-label">{t(item.label as DictKey)}</span>
          </Link>
        );
      })}
    </nav>
  ...
  return (
    <div className="et-surface flex min-h-svh">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {/* Topbar móvil: marca + avatar (perfil) */}
        <header className="sticky top-0 z-30 flex items-center justify-between border-b border-[var(--et-border-soft)] bg-[var(--et-bg-elevated)] px-4 py-3 lg:hidden">
          <Link href="/app" className="et-serif text-base tracking-[0.2em] text-[var(--et-gold-bright)]">{brand}</Link>
          <Link href="/app/perfil" aria-label="Tu perfil"
        ...
        <main className="flex-1 px-4 pb-28 pt-6 sm:px-6 lg:px-10 lg:py-8 lg:pb-8">
