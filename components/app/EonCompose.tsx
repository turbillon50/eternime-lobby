...
export function EonCompose({ firstName = "" }: { firstName?: string }) {
  ...
  return <div className={`eon-chat va-crystal ${hasChat ? "has-chat" : ""}`} data-eon-state={loading ? "thinking" : error ? "error" : "idle"}><LivingMesh/><LightSweep/>
    ...
    {!hasChat && error && <p className="eon-inline-error" role="alert">{error}</p>}
    {!hasChat && !error && <div className="eon-suggestions"><button onClick={()=>setText("¿Qué tengo pendiente?")}>¿Qué tengo pendiente?</button><button onClick={()=>setText("Recuérdame ")}>Crear pendiente</button><button onClick={()=>setText("Busca en mi memoria algo importante que haya olvidado")}>Busca en mi memoria</button></div>}
  </div>;
}
