...
type ApiResponse = { messages?: GuideMessage[]; reply?: string; error?: string };
...
export function EonCompose({ firstName = "" }: { firstName?: string }) {
  ...
  async function send(e: FormEvent) {
    ...
    try {
      ...
      setMessages(m => [...m, { id:`eon-${Date.now()}`, user_id:"eon", role:"assistant", content:reply, created_at:new Date().toISOString() }]);
    } catch {
  ...
  return <div className={`eon-chat ${hasChat ? "has-chat" : ""}`}>
    ...
    {hasChat && <section className="eon-thread" aria-live="polite">
      {messages.slice(-18).map(m => <div key={m.id} className={`eon-message ${m.role}`}><div>{m.content}</div></div>)}
    ...
    <form className="eon-compose-card eon-compose-real" onSubmit={send}>
      <textarea value={text} onChange={e=>setText(e.target.value)} rows={1} placeholder="Pregúntale lo que quieras a Eon…" onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();e.currentTarget.form?.requestSubmit();}}}/>
