...
const FIELD_DEFS: Array<{ key: keyof ProfileUser; label: string; type?: string; placeholder?: string }> = [
  { key: "tagline", label: "Frase / lema", placeholder: "Una frase que te define" },
  { key: "occupation", label: "Ocupación", placeholder: "A qué te dedicas" },
  { key: "birthdate", label: "Fecha de nacimiento", type: "date" },
  { key: "birthplace", label: "Lugar de nacimiento", placeholder: "Ciudad, país" },
  { key: "location", label: "Ubicación actual", placeholder: "Dónde vives" },
...
function FileIcon({ kind }: { kind: StoredFile["kind"] }) {
  ...
  if (kind === "audio") return (<svg {...common}><path d="M9 18V6l10-2v12" /><circle cx="6" cy="18" r="3" /><circle cx="16" cy="16" r="3" /></svg>);
  if (kind === "video") return (<svg {...common}><rect x="3" y="5" width="14" height="14" rx="2" /><path d="m17 9 4-2v10l-4-2" /></svg>);
  if (kind === "document") return (<svg {...common}><path d="M14 3v5h5" /><path d="M14 3H6a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /></svg>);
  return (<svg {...common}><path d="M21 15l-5-5L5 21" /><circle cx="9" cy="9" r="2" /><rect x="3" y="3" width="18" height="18" rx="2" /></svg>);
}
...
export function PerfilClient() {
  ...
  const onAvatar = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setError("");
    try {
      const data = await uploadOne(file, "avatar");
      if (data.user) hydrate(data.user);
    } catch (err) {
  ...
  const onCover = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setError("");
    try {
      const data = await uploadOne(file, "cover");
      if (data.user) hydrate(data.user);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Error al subir");
    } finally {
  ...
  const onFiles = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const file of list) {
        const data = await uploadOne(file, "file");
    ...
    <div className="grid gap-6">
      ...
      <FadeInOnScroll>
        <Card padded={false} className="overflow-hidden">
          <div className="relative">
            ...
            >
              {uploadingCover ? "Subiendo…" : user?.cover_url ? "Cambiar portada" : "Añadir portada"}
            </button>
              ...
              <div className="-mt-12 flex flex-wrap items-end gap-4 sm:-mt-14">
                ...
                <div className="min-w-0 flex-1 pb-1">
                  <div className="flex flex-wrap items-center gap-2">
                    <h2 className="et-serif truncate text-2xl text-[var(--et-text)]">{user?.name ?? "Tu nombre"}</h2>
                  ...
                  <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-[var(--et-text-faint)]">
                    ...
                    {user?.created_at ? <span>· Miembro desde {new Date(user.created_at).getFullYear()}</span> : null}
                  </div>
                </div>
              </div>
            </div>
      ...
      <FadeInOnScroll delay={0.06}>
        <Card>
          ...
          <CardDescription className="mt-1">
            Estos datos guían a tu Eon y dan contexto a tu legado. Llénalos a tu ritmo.
          </CardDescription>

          <div className="mt-5 grid gap-4">
            <Input label="Nombre" value={form.name ?? ""} onChange={(e) => setField("name", e.target.value)} />
            ...
            <Textarea
              label="Biografía"
              rows={5}
              placeholder="Cuenta tu historia: de dónde vienes, qué amas, qué quieres que perdure…"
              value={form.bio ?? ""}
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            ...
            {savedOk ? <span className="text-sm text-[var(--et-success)]">Perfil actualizado</span> : null}
      ...
      <FadeInOnScroll delay={0.1}>
        <Card>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <CardTitle>Tu bóveda de archivos</CardTitle>
              <CardDescription className="mt-1">
                Sube fotos, documentos, audios y videos. Todo lo que quieras preservar.
              </CardDescription>
            </div>
          ...
          {files.length === 0 ? (
            <div className="mt-5">
              <EmptyState
                title="Tu bóveda está esperando su primer recuerdo"
                description="Arrastra aquí o usa “Subir archivos” para guardar fotos, documentos, audios o videos."
                action={<Button variant="ghost" onClick={() => filesInput.current?.click()}>Elegir archivos</Button>}
              />
            </div>
          ) : (
            <div className="mt-5 grid gap-6">
              {images.length > 0 ? (
                <div>
                  <p className="mb-2 text-xs uppercase tracking-[0.14em] text-[var(--et-text-faint)]">Galería</p>
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                    {images.map((f) => (
                      <div key={f.id} className="group relative aspect-square overflow-hidden rounded-[var(--et-radius-sm)] border border-[var(--et-border-soft)]">
                        ...
                        <img src={f.url} alt={f.name ?? "imagen"} className="h-full w-full object-cover transition group-hover:scale-105" />
                        <button
                          type="button"
              ...
              {others.length > 0 ? (
                <div>
                  <p className="mb-2 text-xs uppercase tracking-[0.14em] text-[var(--et-text-faint)]">Documentos y media</p>
                  <div className="grid gap-2">
                    {others.map((f) => (
                      <div key={f.id} className="flex items-center gap-3 rounded-[var(--et-radius-sm)] border border-[var(--et-border-soft)] bg-[rgba(212,175,106,0.03)] px-3 py-2.5">
                        ...
                        <a href={f.url} target="_blank" rel="noopener noreferrer" className="min-w-0 flex-1 truncate text-sm text-[var(--et-text)] hover:text-[var(--et-gold-bright)]">
                          {f.name ?? f.url}
                        </a>
                        <span className="shrink-0 text-xs text-[var(--et-text-faint)]">{fmtSize(f.size)}</span>
                        <button type="button" onClick={() => removeFile(f.id)} aria-label="Eliminar" className="shrink-0 rounded-full p-1 text-[var(--et-text-faint)] transition hover:text-[var(--et-danger)]">
      ...
      <FadeInOnScroll delay={0.16}>
        <Card>
          ...
          <div className="mt-4">
            <Button variant="secondary" onClick={logout} loading={loggingOut}>Cerrar sesión</Button>
          </div>
        </Card>
      </FadeInOnScroll>
      ...
      <FadeInOnScroll delay={0.2}>
        <Card className="!border-[rgba(224,122,106,0.35)]">
          ...
          <CardDescription className="mt-1">
            Eliminar tu cuenta borra para siempre tus recuerdos, cartas, beneficiarios, archivos y conversaciones con tu guía. No hay vuelta atrás.
          </CardDescription>
          <div className="mt-4">
            <Button
              variant="secondary"
      ...
      <Modal open={confirmDelete} onClose={() => setConfirmDelete(false)} title="¿Eliminar tu cuenta?">
        <div className="grid gap-4">
          <p className="text-sm text-[var(--et-text-muted)]">
            Todo tu legado — recuerdos, cartas programadas, beneficiarios, archivos y la memoria de tu guía — se eliminará de forma permanente. Escribe{" "}
            <strong className="text-[var(--et-danger)]">ELIMINAR</strong> para confirmar.
          </p>
          <Input value={deleteText} onChange={(e) => setDeleteText(e.target.value)} placeholder="ELIMINAR" aria-label="Confirmación" />
          <div className="flex justify-end gap-2">
            <Button variant="ghost" onClick={() => setConfirmDelete(false)}>Conservar mi legado</Button>
            <Button
              variant="secondary"
