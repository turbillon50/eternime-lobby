                ...
                {images.map((f) => (
                  <div key={f.id} className="group va-spatial relative aspect-square overflow-hidden rounded-[var(--et-radius-sm)] border border-[var(--et-border-soft)]">
                    ...
                    <button type="button" onClick={() => removeFile(f.id)} aria-label="Quitar archivo"
                      className="absolute right-1.5 top-1.5 rounded-full border border-[var(--eon-edge)] bg-black/75 backdrop-blur-md p-1 text-[var(--et-danger)] opacity-0 transition group-hover:opacity-100 focus-visible:opacity-100">
