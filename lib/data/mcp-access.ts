...
export async function upsertMcpAccess(userId:string,input:{label:string;provider:string;scopes:McpScope[]}){
  const sql=await ensureMcpSchema(); if(!sql)return null;
  const scopes=input.scopes.filter(s=>MCP_SCOPES.includes(s));
  const current=await sql`SELECT id FROM eternime_mcp_access WHERE user_id=${userId} AND provider=${input.provider} AND revoked_at IS NULL ORDER BY created_at DESC`;
  if(!current[0])return createMcpAccess(userId,{...input,scopes});
  const keepId=String(current[0].id);
  const token=`etmcp_${crypto.randomBytes(32).toString("base64url")}`;
  const hash=hashToken(token); const prefix=token.slice(0,12); const encrypted=encryptToken(token);
  const rows=await sql`UPDATE eternime_mcp_access SET label=${input.label},token_hash=${hash},token_prefix=${prefix},token_encrypted=${encrypted},scopes=${scopes}::text[],last_used_at=NULL WHERE id=${keepId} AND user_id=${userId} AND revoked_at IS NULL RETURNING id,user_id,label,provider,token_prefix,scopes,created_at,last_used_at,revoked_at`;
